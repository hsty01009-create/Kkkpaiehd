...
async def add_user(uid, ref=0):
    async with aiosqlite.connect(DB) as db:
        await db.execute("INSERT OR IGNORE INTO users(user_id, invited_by) VALUES(?,?)", (uid, ref))
        await db.commit()
...
async def reward_invite(inviter):
    async with aiosqlite.connect(DB) as db:
        await db.execute("UPDATE users SET coins=coins+200 WHERE user_id=?", (inviter,))
        await db.commit()


async def first_bonus(uid):
    async with aiosqlite.connect(DB) as db:
        await db.execute("""
        UPDATE users
        SET coins=coins+100,
        first_bonus=1
        WHERE user_id=? AND first_bonus=0
        """, (uid,))
        await db.commit()
